...
function AdminPanel() {
  ...
  const currentPlayer =
    auctionState?.current_player_id != null
      ? players.find(
          (p) => String(p.id) === String(auctionState.current_player_id),
        )
      : null;
  ...
  const handlePlaceBid = useCallback(
    async (teamId: bigint) => {
      ...
      try {
        ...
        setOptimisticBid(null);
        setOptimisticLeadingTeamId(null);
        toast.error("Bid failed — network error");
      } finally {
  ...
  const showUndoBid = !!auctionState?.is_active && bidHistory.length > 0;
  const showUnsell = !auctionState?.is_active && lastSoldPlayer !== null;
              ...
              {upcomingPlayers.map((player) => (
                ...
                >
                  ...
                  <button
                    ...
                    disabled={auctionState?.is_active}
            ...
            >
              ...
              <button
                ...
                style={{
                  fontSize: "18px",
                  background:
                    auctionState?.is_active && auctionState.leading_team_id
